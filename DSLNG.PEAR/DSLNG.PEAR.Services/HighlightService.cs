...
namespace DSLNG.PEAR.Services
{
    public class HighlightService  : BaseService,IHighlightService
    {
        ...
        private IEnumerable<Highlight> SortData(string search, IDictionary<string, SortOrder> sortingDictionary, out int TotalRecords)
        {
            var data = DataContext.Highlights.Include(x => x.HighlightType).AsQueryable();
            if (!string.IsNullOrEmpty(search) && !string.IsNullOrWhiteSpace(search))
            {
                data = data.Where(x => x.HighlightType.Value.Contains(search) || x.Title.Contains(search) || x.Type.Contains(search));
            }
