...
namespace DSLNG.PEAR.Web.Scheduler
{
    ...
    {
        public static List<IJob> Jobs = new List<IJob>();
        public static List<IJob> RunningJobs = new List<IJob>();
    ...
    public class QueueJob : IJob
    {
        public void Execute()
        {
            if(JobList.RunningJobs.Count == 0)
            {
                if(JobList.Jobs.Count > 0)
                {
                    var job = JobList.Jobs[JobList.Jobs.Count - 1];
                    JobList.Jobs.RemoveAt(JobList.Jobs.Count - 1);
                    JobManager.AddJob(job, s => s.ToRunNow());
                    JobList.RunningJobs.Add(job);
                }
            }
        }
    }
    ...
    public class MyJob : IJob, IRegisteredObject
    {
        ...
        private Data.Entities.BaseAction action;
        SaveKpiTransformationScheduleResponse kpiTransformationSchedule;
        public MyJob(Data.Entities.BaseAction action, string kpiPattern, SaveKpiTransformationScheduleResponse kpiTransformationSchedule)
        {
            this.kpiPattern = kpiPattern;
            this.action = action;
            this.kpiTransformationSchedule = kpiTransformationSchedule;
        }
        ...
        public void Execute()
        {
            ...
            using (var dataContext = new DataContext())
            {
                ...
                JobList.RunningJobs.RemoveAt(0);
    ...
    public class KpiTransformationJob : IKpiTransformationJob
    {
        public void Process(SaveKpiTransformationScheduleResponse kpiTransformationSchedule)
        {
            ...
            var kpiPattern = @"k(\d+)";
            var j = new MyJob(action, kpiPattern, kpiTransformationSchedule);

            JobList.Jobs.Add(j);

            //Queue<MyJob> q = new Queue<MyJob>();
            //q.Enqueue(j);

            //while (q.Count > 0)
            //{
            //    JobManager.AddJob(j, s => s.NonReentrant().ToRunNow());
            //}
        }
        private DateTime Increment(SaveKpiTransformationScheduleResponse kpiTransformationSchedule, DateTime periode)
        {
